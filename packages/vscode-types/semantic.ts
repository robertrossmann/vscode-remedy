...
import { type TMRuleSetting } from './textmate.js'
...
type SemanticToken = `${SemanticTokenType}${'' | `.${SemanticTokenModifier}`}${'' | `:${SemanticTokenLanguage}`}`

type SemanticRules = Partial<Record<SemanticToken, SemanticRule | TMRuleSetting | ColorInstance>>
